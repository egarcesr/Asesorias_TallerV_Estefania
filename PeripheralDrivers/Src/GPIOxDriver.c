 ...
 *
 *  Este archivo es la parte más importante donde escribimos adecuadamente el control,
 *  para que sea lo más genérico posible, de forma que independiente del puerto GPIO y
 *  del PIN seleccionados, el programa se ejecute y configure todo correctamente.
 *
 */
 ...
 * Lo primero y más impportante es activar la señal del reloj principal hacia ese
...
void GPIO_Config (GPIO_Handler_t *pGPIOHandler){
	...
	// De nuevo, leemos y modemos el valor un número "PinNumber" de veces
	...
	// 5) Configurando si se desea pull-ip, pull-down o flotante
...
void GPIO_WritePin (GPIO_Handler_t *pPinHandler, uint8_t newState){
	...
	// pPinNumber -> pGPIOx -> ODR &= ~(SET << pPinHandler -> GPIO_PinConfig.GPIO_PinNumber);
	if (newState == SET){
