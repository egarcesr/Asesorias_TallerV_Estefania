...
void USART_Config(USART_Handler_t *ptrUsartHandler){
	...
	if (ptrUsartHandler -> USART_Config.USART_parity != USART_PARITY_NONE){
		// Detección/generación de paridad activado
...
int writeChar(USART_Handler_t *ptrUsartHandler, int dataToSend ){
	// Verificar si el dato se transmitió 1 o si no se ha transmitido 0, si está ocupado (uno es 0 y el otro 1
	// el and es un false y con el ! es un true, entra al while y no hace nada
	while( !(ptrUsartHandler -> ptrUSARTx -> SR & USART_SR_TXE)){
		__NOP();
	}
	// Si está desocupado mande el char
